...
class BookingsController extends Controller
{
    ...
    {
        $module_title = 'menu.booking_calendar';
        $module_action = 'List';
        ...
        return view('booking::backend.bookings.index', compact('module_title', 'module_action', 'statusList', 'date'));
    }
    ...
    public function statusList()
    {
        ...
        foreach ($booking_status as $key => $value) {
            ...
            if ($value->name !== 'cancelled') {
                $color = $bookingColors->where('sub_type', $value->name)->first();
                        $statusList[$value->name] = [
                            'title' => $value->value,
                            'color_hex' => $color ? $color->name : '#000000', // Default color if not found
    ...
    public function index_list(Request $request)
    {
        ...
        $booking_type=$userTypeToBookingType[$userType] ?? '';
        $data = Booking::where('booking_type',$booking_type)->with( 'employee', 'service')->get();
        $updated_data = [];
        $statusList = $this->statusList();
        foreach ($data as $key => $value) {
            $duration = $value->duration_min;
            $startTime = $value->start_date_time;
            $relatedData = $this->fetchRelatedData($value,$startTime, $booking_type);
            $serviceName = $relatedData['service_name'] ?? 'Unknown Service';
            $duration = $relatedData['endTime'] ??$duration;
            $customerName = optional($value->user)->first_name.' '.optional($value->user)->last_name ?? 'Anonymous';
            $endTime= $relatedData['endTime'];
            ...
            $updated_data[$key] = [
                ...
                'color' => $statusList[$value->status]['color_hex']??'#D1D1D1',
            ];
            $startTime = $endTime;
        }
        // $employees = User::bookingEmployeesList()->get();
        $employeesQuery = User::employee($userType)->paginate($perPage, ['*'], 'page', $page);
        $employees = $employeesQuery->items();
        $resource = [];
        ...
        return response()->json([
            'data' => $updated_data,
            'employees' => $resource,
            'total_count' => $employeesQuery->total(),
        ]);
    }
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('pick_date_time', function ($data) {
                return date('d-m-Y h:i A', strtotime(optional($data->boarding)->pickup_date_time));
            })
            ...
            ->editColumn('drop_date_time', function ($data) {
                return date('d-m-Y h:i A', strtotime(optional($data->boarding)->dropoff_date_time));
            })
        ...
        switch ($booking['booking_type']) {
            ...
            case 'veterinary':
                ...
                if (isset($data['address'])) {
                    $booking_mapping['address'] = $data['address'];
                }
    ...
    public function show($id)
    {
        ...
        if (method_exists(Booking::class, $booking_type)) {
            $booking = Booking::with([$booking_type, 'user'])->find($id);
        } else {
        ...
        $bookingTransaction = BookingTransaction::where('booking_id', $booking->id)->first();


        switch ($booking_type) {
            case 'boarding':
                $bookingResource = new BookingBoardingResource($booking);
                break;
            case 'veterinary':
                $bookingResource = new BookingVeterinaryResource($booking);
                break;
            case 'grooming':
                $bookingResource = new BookingGroomingResource($booking);
                break;
            case 'training':
                $bookingResource = new BookingTrainerResource($booking);
                break;
            case 'walking':
                $bookingResource = new BookingWalkerResource($booking);
                break;
            case 'daycare':
                $bookingResource = new BookingDaycareResource($booking);
            default :
        ...
        $data = [
            ...
            'services_total_amount' => $booking->service_amount,
    ...
    public function edit($id)
    {
        ...
        switch ($booking['booking_type']) {
            case 'boarding':
                ...
                $data['price'] = $data->boarding->price;
            ...
            case 'veterinary':
                ...
                $data['price'] = $data->veterinary->price;
        ...
        $bookingtransaction = BookingTransaction::where('booking_id', $id)->first();

        if($bookingtransaction){
            $taxData = json_decode($bookingtransaction->tax_percentage);
            $total_tax_amount = 0;
            foreach ($taxData as $tax) {
                if($tax->type == 'percentage'){
                    $total_tax_amount += ($data['price'] * $tax->value) / 100;
                }else{
                    $total_tax_amount += $tax->value;
                }
            }
            $data['total_tax'] = $total_tax_amount;
        }
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        switch ($status) {
            case 'confirmed':
                ...
                $messageTemplate = 'Booking #[[booking_id]] has been accepted.';
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'rejected':
                $notify_type = 'reject_booking';
                $messageTemplate = 'Booking #[[booking_id]] has been rejected.';
                $notify_message = str_replace('[[booking_id]]', $id, $messageTemplate);
                break;
            case 'completed':
    ...
    public function bookingShow($id)
    {
        ...
        $bookingColors = Constant::getAllConstant()
            ->where('type', 'BOOKING_STATUS_COLOR')
            ->where('sub_type', $booking->status)
            ->pluck('name')
            ->toArray();
        $statusColor = count($bookingColors) > 0 ? reset($bookingColors) : '#5c95f7'; 
        // dd(['status' => $booking->status, 'statusColor' => $statusColor]);
        ...
        return view('booking::backend.bookings.booking', compact('booking', 'tax', 'module_title', 'statusColor'));
    }
    ...
    protected function fetchRelatedData($booking,$startTime, $bookingType)
{
    switch ($bookingType) {
        case 'boarding':
            $relatedData = BookingBoardingMapping::where('booking_id', $booking->id)->first();
            $endTime=$relatedData->pickup_date_time;
            return [
        ...
        case 'veterinary':
            $relatedData = BookingVeterinaryMapping::where('booking_id', $booking->id)->first();
            return [
                'service_name' => $relatedData->service_name ?? 'Veterinary Service',
                'duration' => $relatedData->duration ?? null,
                'endTime' => Carbon::parse($startTime)->addMinutes($relatedData->duration)
            ];
        case 'grooming':
            $relatedData = BookingGroomingMapping::where('booking_id', $booking->id)->first();
            return [
                'service_name' => $relatedData->service_name ?? 'Grooming Service',
                'duration' => $relatedData->duration ?? null,
                'endTime' => Carbon::parse($startTime)->addMinutes($relatedData->duration)
            ];
        case 'training':
            $relatedData = BookingTrainerMapping::where('booking_id', $booking->id)->first();
            list($hours, $minutes) = explode(':', $relatedData->duration);
        ...
        case 'walking':
            $relatedData = BookingWalkerMapping::where('booking_id', $booking->id)->first();
            list($hours, $minutes) = explode(':', $relatedData->duration);
            $totalMinutes = ($hours * 60) + $minutes;
            return [
                'service_name' => 'Training Service',
        ...
        case 'daycare':
            $relatedData = BookingDayCareMapping::where('booking_id', $booking->id)->first();
            $date =  (new DateTime($relatedData->date))->format('Y-m-d') ?? Carbon::now()->format('Y-m-d');
            $pickUpTime = $relatedData->pickup_time ?? '00:00:00';
            $dateTimeString = "{$date} {$pickUpTime}";
            $endTime = Carbon::parse($dateTimeString);
            return [
                'service_name' => 'Daycare Service',
                'endTime' => $endTime->format('Y-m-d H:i:s'),
                'duration' => null, // Duration might not be applicable for daycare in this context
            ];
        default:
            return [
