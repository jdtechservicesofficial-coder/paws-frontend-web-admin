                ...
                <div class="d-flex justify-content-between flex-wrap">
                    ...
                    <div class="d-flex gap-5 align-items-center">
                        <div class="flex-column">
                            ...
                            <h6 class="m-0"> <img
                                    src="{{ optional($booking->pet)->pet_image ?? default_user_avatar() }}" alt="avatar"
                                    class="avatar avatar-30 rounded-pill">
                                {{ $booking->pet ? optional($booking->pet)->name : '--' }}</h6>
                        </div>
                        <div class="flex-column">
                            <span class="mb-2">{{ __('pet.pet_breed') }}</span>
                            <h6 class="m-0">
                                {{ $booking->pet ? optional(optional($booking->pet)->breed)->name : '--' }}
                            </h6>
                        </div>
                        <div class="flex-column">
                            <span class="mb-2">{{ __('booking.lbl_status') }}</span>
                            <h6 class="m-0" style="color: {{ $statusColor }}">{{ ucfirst($booking->status) }}</h6>
                        </div>
        ...
        <div class="card card-block card-stretch card-height">
            <div class="card-body">
                <div class="row gap-5 mb-5">
                    ...
                    <div class="col-md-3">
                        @if ($booking->booking_type === 'boarding')
                            ...
                            @if ($booking->employee === null)
                                <h6 class="m-0">--</h6>
                            @else
                                <h6 class="m-0"> <img src="{{ $booking->employee->profile_image ?? default_user_avatar() }}"
                    ...
                    <div class="col-md-3">
                        @if ($booking->medical_report && !str_contains($booking->medical_report, 'default.png'))
                            <span>{{ __('booking.lbl_medical_report') }}</span>
                            <div class="mt-4">
                                <a href="{{ $booking->medical_report }}" class="btn btn-primary" download>
                                    <i class="fas fa-file-download"></i>
                                    {{ __('Download Medical Report') }}
                                </a>
                            </div>
                        @endif
                    </div>
                ...
                <div class="row gap-5 pt-5">
                    <div class="col-md-3">
                        ...
                        @if (isset($booking->payment->payment_status))
                            @if ($booking->payment->payment_status == 0)
                                <p class="m-0 text-warning">{{ __('booking.pending') }}</p>
                            @else
                                <h6 class="mb-4 text-success">{{ __('booking.paid') }}</h6>
                    ...
                    <div class="col-md-3 mt-4 mt-md-0">
                        <span>Address / Dropoff & Pickup</span>
                        @if ($booking->booking_type === 'boarding' && isset($booking->boarding->dropoff_address))
                            <h6 class="m-0">Dropoff: {{ $booking->boarding->dropoff_address }}</h6>
                            @if(isset($booking->boarding->pickup_address))
                                <h6 class="m-0 mt-2">Pickup: {{ $booking->boarding->pickup_address }}</h6>
                            @endif
                        @elseif ($booking->booking_type === 'walking' && isset($booking->walking->address))
                            <h6 class="m-0">{{ $booking->walking->address }}</h6>
                        @elseif ($booking->booking_type === 'daycare' && isset($booking->daycare->address))
                            <h6 class="m-0">{{ $booking->daycare->address }}</h6>
                        @elseif ($booking->booking_type === 'grooming' && isset($booking->grooming->address) && $booking->grooming->address != '')
                            <h6 class="m-0">{{ $booking->grooming->address }} (Drop off & Pick up)</h6>
                        @elseif ($booking->booking_type === 'veterinary' && isset($booking->veterinary->address) && $booking->veterinary->address != '')
                            <h6 class="m-0">{{ $booking->veterinary->address }} (Drop off & Pick up)</h6>
                        @else
                            <h6 class="m-0">--</h6>
                        @endif
                    </div>
