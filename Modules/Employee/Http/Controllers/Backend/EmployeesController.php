...
class EmployeesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $commissions_list = Commission::where('type', 'service')->get();
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->orderColumn('one_day_price', function ($query, $order) {
                $query->leftJoin('boarder_daycare_perdayamount AS bda', 'bda.user_id', '=', 'users.id') // Join with boarder_daycare_amounts table
                    ->leftJoin('settings AS s_boarder', function ($join) {
                        $join->on('s_boarder.name', '=', DB::raw('"pet_boarding_amount"'));
                    })
                    ->leftJoin('settings AS s_day_taker', function ($join) {
                        $join->on('s_day_taker.name', '=', DB::raw('"pet_daycare_amount"'));
                    })
                    ->orderByRaw("
                          COALESCE(
                              bda.amount,
                              CASE
                                  WHEN users.user_type = 'boarder' THEN s_boarder.val
                                  WHEN users.user_type = 'day_taker' THEN s_day_taker.val
                                  ELSE 0
                              END
                          ) $order
                      ");
    ...
    public function destroy($id)
    {
        ...
        $booking = Booking::where('employee_id', $id)->where('status', '!=', 'completed')->update(['status' => 'cancelled']);
        $data->tokens()->delete();
        $data->forceDelete();
    ...
    public function update_status(Request $request, $id)
    {
        $user = User::find($id);
        if (!$user) {
            return response()->json(['status' => false, 'message' => __('messages.user_not_found') ?? 'Employee not found. Please refresh the page.']);
        }
    ...
    {
        $data = User::role(['vet', 'groomer', 'walker', 'boarder', 'trainer', 'day_taker', 'pet_sitter', 'pet_store'])->find($id);
