          ...
          <div class="d-grid">
            ...
            <div v-if="selectedpets.length > 0" class="list-group list-group-flush">
              <div v-for="(item, index) in selectedpets" :key="item" class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <button  type="button" class="fs-4 text-info border-0 bg-transparent" data-bs-toggle="offcanvas" data-bs-target="#pet-notes-form"    @click="changeId(item.id)"  aria-controls="form-offcanvas" title="View Notes" ><i class="icon-page"></i></button>
                <button v-if="role() === 'admin' || role() === 'demo_admin'" type="button" class="fs-4 text-primary border-0 bg-transparent" data-bs-toggle="offcanvas" data-bs-target="#pet-form-offcanvas"    @click="changeId(item.id)"  aria-controls="form-offcanvas" title="Edit" ><i class="icon-Edit"></i></button>
                <button v-if="role() === 'admin' || role() === 'demo_admin'" class="fs-4 text-danger border-0 bg-transparent" @click="removePet(item.id, 'Are you certain you want to delete it?')"  title="Delete" data-bs-toggle="tooltip"> <i class="icon-delete"></i></button>
