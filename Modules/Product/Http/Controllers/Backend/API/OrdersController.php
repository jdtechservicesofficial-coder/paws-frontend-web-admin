   ...
   use CommissionCalculatorTrait;
    ...
    public function store(OrderRequest $request)
    {
        ...
        if (count($carts) > 0) {
            ...
            $order->logistic_id                     = $logisticZone ? $logisticZone->logistic_id : null;
            $order->logistic_name                   = $logisticZone ? optional($logisticZone->logistic)->name : null;
    ...
    public function  orderList(Request $request){
        ...
        if ($request->has('search') && !empty($request->search)) {
            $search = $request->search;
             $orderQuery->whereHas('product_variation.product', function ($query) use ($search) {
