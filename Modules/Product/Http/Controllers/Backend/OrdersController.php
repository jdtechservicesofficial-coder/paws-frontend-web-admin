    ...
    public function updateDeliveryStatus(Request $request)
    {
        ...
        switch ($status) {
            case 'accept':
                ...
                $messageTemplate = 'Order #[[order_id]] has been accepted.';
...
public function downloadInvoice(Request $request, $id = null)
{
    ...
    $otherItems = OrderItem::with('order')
    ...
    // Prepare PDF view
    $view = view('product::backend.order.invoice', compact(
        'orderItem',
        'otherItems',
        'orderUpdates',
        'checkVendor',
    ...
    if (!class_exists(\Barryvdh\DomPDF\Facade\Pdf::class)) {
        \Log::error('DomPDF is not installed. Please run composer require barryvdh/laravel-dompdf');
        return response()->json(['status' => false, 'message' => 'PDF package missing'], 404);
    }

    $pdf = Pdf::loadHTML($view)->setPaper('a4', 'portrait');

    // For API request → return file URL
    if ($request->is('api/*')) {
        $baseDirectory = storage_path('app/public');
        $nextDirectory = now()->timestamp;
        File::makeDirectory($baseDirectory . '/' . $nextDirectory, 0777, true);

        $filename = 'invoice_' . $orderItem->id . '.pdf';
        $filePath = $baseDirectory . '/' . $nextDirectory . '/' . $filename;
        $pdf->save($filePath);
    ...
    return response()->streamDownload(
        fn () => print($pdf->output()),
        setting('inv_prefix') . optional(optional($orderItem->order)->orderGroup)->order_code . '.pdf',
