    ...
    public function index_data(Request $request)
    {
        ...
        return Datatables::of($query)
                        ...
                        ->editColumn('min_price', function ($data) {
                            ...
                            if ($data->discount_start_date && $data->discount_end_date && $data->discount_value != 0) {
                                ...
                                if ($discountStartDate->lte($now) && $discountEndDate->gte($now)) {
                                    ...
                                    if ($data->max_price != $data->min_price) {
                                        $formattedPrice = \Currency::format($discount_min_price) . ' <span style="text-decoration: line-through;">' . \Currency::format($data->min_price) . '</span> - ' . \Currency::format($discount_max_price) . ' <span style="text-decoration: line-through;">' . \Currency::format($data->max_price) . '</span>';
                                    } else {
                                        $formattedPrice = \Currency::format($discount_min_price) . ' <span style="text-decoration: line-through;">' . \Currency::format($data->min_price) . '</span>';
                                    }
                                } else {
                                    if ($data->max_price != $data->min_price) {
                                        $formattedPrice = \Currency::format($data->min_price) . ' - ' . \Currency::format($data->max_price);
                                    } else {
                                        $formattedPrice = \Currency::format($data->min_price);
                                    }
                                }
                            } else {
                                if ($data->max_price != $data->min_price) {
                            ...
                            // Apply the ellipsis if the price string exceeds a certain length
                            $maxLength = 20; // Max number of characters before ellipsis
                            if (strlen($formattedPrice) > $maxLength) {
                                $displayPrice = substr($formattedPrice, 0, $maxLength) . '...';
                                return '<span data-bs-toggle="tooltip" title="' . $formattedPrice . '">' . $displayPrice . '</span>';
                            }
    ...
    public function edit($id)
    {
        ...
        if ($data->has_variation) {
            ...
            foreach ($combinations as $key => $value) {
                $stock_record = $value->product_variation_stock()->where('location_id', $location->id)->first();
                $combinations[$key] = [
                    'product_id' => $value->product_id,
                    'variation' => $value->sku,
                    'variation_key' => $value->variation_key,
                    'stock' => $stock_record ? $stock_record->stock_qty : 0,
                    'code' => $value->code,
                    'sku' => $value->sku,
                    'price' => $value->price,
                ];
            ...
            $stock_record = $variation ? $variation->product_variation_stock()->where('location_id', $location->id)->first() : null;
            $data['stock'] = $stock_record ? $stock_record->stock_qty : 0;
            $data['code'] = $variation ? $variation->code : '';
            $data['price'] = $variation ? $variation->price : 0;
            $data['sku'] = $variation ? $variation->sku : '';
        }
