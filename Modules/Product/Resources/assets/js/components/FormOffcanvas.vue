      ...
      <div class="offcanvas-body">
        ...
        <fieldset>
              ...
              <div class="col-md-12">
                <div class="d-flex gap-3 align-items-center">
                  ...
                  <div v-if="variations.length > 1">
                    <button class="btn btn-danger btn-icon" @click="handleButtonClick(index)">
          ...
          <div class="row" v-else>
            ...
            <InputField class="col-md-3" type="number" :step="0" :min="1"  :max="1000"  :label="$t('product.stock')" placeholder="" v-model="stock" :is-required="true" :error-message="errors['stock']"></InputField>
...
const fileUpload = async (e) => {
  ...
  errorMessages.value.feature_image = [];
  const reader = new FileReader();
  reader.onload = () => {
    const img = new Image();
    img.onload = () => {
      ImageViewer.value = reader.result;
      profileInputRef.value.value = ''; 
      feature_image.value = file; 
    };
    img.onerror = () => {
      errorMessages.value.feature_image = ["This image is corrupted, Please select another file!"];
    };
...
const reset_datatable_close_offcanvas = (res) => {
  ...
  if (res.status) {
    ...
    errorMessages.value = res.all_message || res.errors || {}
  }
}
