...
@section('title')
    {{ setting('inv_prefix') }}{{ optional(optional($orderItem->order)->orderGroup)->order_code }}
@endsection
        ...
        <div class="col-12">
            <div class="card ">
                <div class="card-header border-bottom-0">
                    <div class="row pr-hide">
                        @php
                            $expected_delivery_date = \Carbon\Carbon::parse($orderItem->expected_delivery_date)->format(
                                'd-m-Y',
                            );
                        @endphp
                            ...
                            <div class="col-auto col-lg-4 mb-4">
                                <div class="d-flex">
                                    ...
                                    <input type="text" name="expected_delivery_date"
                                        value="{{ $expected_delivery_date }}" id="expected_delivery_date"
                                        placeholder="Select Date" class="expected_delivery_date form-control" readonly />
                                </div>
                            </div>
                                    ...
                                    <div class="input-group">
                                        <select class="form-select select2" name="payment_status"
                                            ...
                                            <option value="pending"
                                                {{ $orderItem->payment_status == 'pending' ? 'selected' : '' }}>
                                                Pending
                                            </option>

                                            @if ($orderItem->delivery_status == 'delivered')
                                                <option value="paid"
                                                    {{ $orderItem->payment_status == 'paid' ? 'selected' : '' }}>
                                                    {{ __('product.lbl_paid') }}
                                                </option>
                                            @endif

                                            <option value="unpaid"
                                                {{ $orderItem->payment_status == 'unpaid' ? 'selected' : '' }}>
                                                {{ __('product.unpaid') }}
                                            </option>
                                        </select>
                                    </div>
                        ...
                        <div class="col-auto flex-grow-1">
                            ...
                            <div class="welcome-message">
                                <h5 class="mb-2">Customer Info</h6>
                                    <p class="mb-0">Name:
                                        <strong>{{ optional(optional($orderItem->order)->user)->full_name }}</strong></p>
                                    <p class="mb-0">Email:
                                        <strong>{{ optional(optional($orderItem->order)->user)->email }}</strong></p>
                                    <p class="mb-0">Phone:
                                        <strong>{{ optional(optional($orderItem->order)->user)->mobile }}</strong></p>
                            </div>
                        </div>
                    ...
                    <div class="d-flex justify-content-md-between justify-content-center g-3">
                        <div class="col-auto">
                            ...
                            <div class="col-auto mt-3">
                                ...
                                <span
                                    class="badge bg-primary">{{ ucwords(str_replace('_', ' ', optional(optional($orderItem->order)->orderGroup)->payment_method)) }}</span>
                            </div>
                            <h6 class="col-auto d-inline-block">{{ __('product.logistic') }} </h6> <span
                                class="badge bg-primary">{{ $orderItem->order->logistic_name }}</span>
                        </div>
                        <div class="col-auto">
                            <div class="shipping-address d-flex justify-content-md-end gap-3 mb-3">
                                <div class="border-end w-25">
                                    @php
                                        $shippingAddress = optional(optional($orderItem->order)->orderGroup)
                                            ->shippingAddress;
                                    @endphp
                                ...
                                @if (!$orderItem->order->orderGroup->is_pos_order)
                                    <div class="w-25">
                                        @php
                                            $billingAddress = optional(optional($orderItem->order)->orderGroup)
                                                ->billingAddress;
                                        @endphp
                    ...
                    <tfoot class="text-end">
                        ...
                        <tr>
                            ...
                            <td width="10%" class="text-end">
                                <strong>{{ \Currency::format($orderItem->total_tax) }}</strong>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td width="10%" class="text-end">
                                <strong>{{ \Currency::format($orderItem->total_shipping_cost) }}</strong>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="4">
                        ...
                        <table class="table table-bordered border-top order_table" data-use-parent-width="true">
                            ...
                            <tfoot class="text-end">
                                ...
                                <tr>
                                    ...
                                    <td width="10%" class="text-end">
                                        <strong>{{ \Currency::format($item->total_tax) }}</strong>
                                    </td>
                                </tr>
                        ...
                        @if (!auth()->user()->hasRole('pet_store'))
                            <tr>
                                ...
                                <td width="10%" class="text-end" style="border-style: none;">
                                    <strong
                                        class="text-accent">{{ \Currency::format(optional(optional($orderItem->order)->orderGroup)->grand_total_amount) }}</strong>
                                </td>
                            ...
                            @forelse ($orderUpdates as $orderUpdate)
                                <li>
                                    <a class="{{ $loop->first ? 'active' : '' }}">
                                        ...
                                        <span
                                            class="text-capitalize">{{ optional($orderUpdate->user)->first_name }}</span>
                                        at
