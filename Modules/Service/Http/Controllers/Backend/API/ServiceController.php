...
use Modules\BussinessHour\Traits\BussinessHourTrait;

class ServiceController extends Controller
{
    use BussinessHourTrait;

    public function store(ServiceRequest $request)
    {
        $data = $request->except(['feature_image', 'branches', 'staffs', 'duration_min']);
        $type = $request->type;
        $data['type'] = $type;
        $duration = $request->duration_min ?? 0;
        ...
        if ($request->has('branches')) {
            $branches = is_string($request->branches) ? json_decode($request->branches, true) : $request->branches;
            if (is_array($branches)) {
                foreach ($branches as $branch) {
                    ServiceBranches::create([
                        'service_id' => $query->id,
                        'branch_id' => $branch['branch_id'] ?? (is_array($branch) ? $branch['id'] ?? $branch : $branch),
                        'service_price' => $data['default_price'] ?? 0,
        ...
        if (auth()->user()->hasAnyRole(['admin', 'demo_admin'])){
            if ($request->has('staffs')) {
                $staffs = is_string($request->staffs) ? json_decode($request->staffs, true) : $request->staffs;
                if (is_array($staffs)) {
                    foreach ($staffs as $staff) {
                        ServiceEmployee::create([
                            'service_id' => $query->id,
                            'employee_id' => $staff['employee_id'] ?? (is_array($staff) ? $staff['id'] ?? $staff : $staff),
                        ]);
                    }
                }
            }
    ...
    public function update(ServiceRequest $request)
    {
        ...
        $request_data = $request->except(['feature_image', 'branches', 'staffs', 'duration_min']);
        $duration = $request->duration_min ?? 0;
            ...
            if (is_array($branches)) {
                foreach ($branches as $branch) {
                    ServiceBranches::create([
                        'service_id' => $id,
                        'branch_id' => $branch['branch_id'] ?? (is_array($branch) ? $branch['id'] ?? $branch : $branch),
                        'service_price' => $request_data['default_price'] ?? $data->default_price ?? 0,
                        'duration_min' => $duration,
                    ]);
                }
            }
    ...
    public function serviceList(Request $request)
    {
        ...
        $service =  Service::with(['category','employee']);
        ...
        $workingDays = $this->getWorkingDays($branchId);
        return response()->json([
            'status' => true,
            'data' => $responseData,
    ...
    {
        $query = SystemService::with(['media'])->orderBy('id', 'asc');
        ...
        return response()->json([
            'status' => true,
            'data' => SystemServiceResource::collection($data),
            'message' => 'System services retrieved successfully',
        ], 200);
    }

    public function updateSystemServiceStatus(Request $request, $id)
    {
        $service = SystemService::find($id);
        if (!$service) {
            return response()->json(['status' => false, 'message' => 'System service not found'], 404);
        }
    ...
    public function updateSystemService(Request $request, $id)
    {
        $service = SystemService::find($id);
        if (!$service) {
            return response()->json(['status' => false, 'message' => 'System service not found'], 404);
        ...
        if ($request->has('status')) $service->status = $request->status ? 1 : 0;
        $service->save();
        ...
        return response()->json([
            ...
            'data' => new SystemServiceResource($service->fresh()),
        ], 200);
    }
}
