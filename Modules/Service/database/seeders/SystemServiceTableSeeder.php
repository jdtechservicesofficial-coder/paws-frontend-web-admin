...
class SystemServiceTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $existingService = SystemService::where('slug', $value['slug'])->first();
            if (!$existingService) {
                $createdService = SystemService::create($service);
                if (isset($featureImage)) {
                    try {
                        $this->attachFeatureImage($createdService, $featureImage);
                    } catch (\Exception $e) {
                        // Keep going if media attachment fails in production
                    }
                }
            }
        }
