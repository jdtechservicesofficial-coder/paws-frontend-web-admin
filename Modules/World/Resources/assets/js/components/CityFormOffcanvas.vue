    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="form-offcanvas" aria-labelledby="form-offcanvasLabel">
      ...
      <div class="offcanvas-body">
        ...
        <div class="col-md-12 form-group">
          ...
          <span class="text-danger" v-if="showCountryFirstError"></span>
        </div>
...
const setFormData = (data) => {
  resetForm({
    values: {
      name: data.name,
      status: data.status,
      country_id: data?.city?.country?.id,
...
const validationSchema = yup.object({
  ...
  state_id: yup.string().required('State is a required field').test(
    'countryFirst',
    'Please select a country first',
    function(value) {
      return !!this.parent.country_id;
    }
  ),
...
const showCountryFirstError = ref(false)

watch([country_id, state_id], ([newCountry, newState]) => {
  if (!newCountry && newState) {
    showCountryFirstError.value = true
  } else {
    showCountryFirstError.value = false
  }
})
