...
class BackendController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $topproduct = ProductCategory::orderByDesc('total_sale_count')->limit(5)->get();
    ...
    public function getRevenuechartData($type){
        ...
        if($type =='year'){
            ...
            ->select(DB::raw('YEAR(bookings.start_date_time) as year, MONTH(bookings.start_date_time) as month, SUM(bookings.total_amount) as total_amount'))
            ->where('bookings.status', 'completed')
            ->where('booking_transactions.payment_status', 1)  // Filter by successful payment status
            ->groupBy(DB::raw('YEAR(bookings.start_date_time), MONTH(bookings.start_date_time)'))
            ->orderBy(DB::raw('YEAR(bookings.start_date_time), MONTH(bookings.start_date_time)'))
            ->get();

            $orderTotals = DB::table('order_groups')
            ->select(DB::raw('YEAR(created_at) as year, MONTH(created_at) as month, SUM(grand_total_amount) as total_amount'))
            ...
            ->get();
              ...
              for ($month = 1; $month <= 12; $month++) {
                  $total = 0;
                  foreach ($monthlyTotals as $b_total) {
                      if ((int)$b_total->month === $month) {
                          $total += (float)$b_total->total_amount;
                      }
                  }
                  foreach ($orderTotals as $o_total) {
                      if ((int)$o_total->month === $month) {
                          $total += (float)$o_total->total_amount;
                      }
                  }
                  $chartData[] = $total;
              };
           ...
           $category =["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct",
            ...
            $orderWeekTotals = DB::table('order_groups')
                ->select(DB::raw('YEAR(created_at) as year, MONTH(created_at) as month, WEEK(created_at) as week, COALESCE(SUM(grand_total_amount), 0) as total_amount'))
                ->where('payment_status', 'paid')
            ...
            for ($i = $firstWeek; $i <= $firstWeek+4; $i++) {
                $total = 0;
                foreach ($monthlyWeekTotals as $b_total) {
                    if ((int)$b_total->month === $currentMonth && (int)$b_total->week === $i) {
                        $total += (float)$b_total->total_amount;
                    }
                }
                foreach ($orderWeekTotals as $o_total) {
                    if ((int)$o_total->month === $currentMonth && (int)$o_total->week === $i) {
                        $total += (float)$o_total->total_amount;
                    }
                }
                $chartData[] = $total;
            ...
            $orderDayTotals = DB::table('order_groups')
             ->select(DB::raw('DAY(created_at) as day, COALESCE(SUM(grand_total_amount), 0) as total_amount'))
             ->where('payment_status', 'paid')
             ->where(DB::raw('YEAR(created_at)'), '=', Carbon::now()->year)
             ->where(DB::raw('MONTH(created_at)'), '=', $currentMonth)
             ...
             for($day =  $currentWeekStartDate; $day <= $lastDayOfWeek; $day->addDay()) {
                 $total = 0;
                 foreach ($weeklyDayTotals as $b_total) {
                     if ((int)$b_total->day === $day->day) {
                         $total += (float)$b_total->total_amount;
                     }
                 }
                 foreach ($orderDayTotals as $o_total) {
                     if ((int)$o_total->day === $day->day) {
                         $total += (float)$o_total->total_amount;
                     }
                 }
                 $chartData[] = $total;
