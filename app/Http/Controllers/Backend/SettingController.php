...
class SettingController extends Controller
{
    ...
    {
        \Log::info('SettingController store payload: ' . json_encode($request->all()));
        $data = $request->all();
        ...
        if (isset($data['app_domain_url'])) {
            try {
                $this->updateFirebaseRemoteConfig($data['app_domain_url']);
            } catch (\Exception $e) {
                \Log::error('Firebase Remote Config Update Failed: ' . $e->getMessage());
            }
        }
    ...
    private function updateFirebaseRemoteConfig($domainUrl)
    {
        $credentialsPath = base_path('firebase-service-account.json');
        if (!file_exists($credentialsPath)) {
            \Log::warning('Firebase service account file not found at ' . $credentialsPath . '. Remote config update skipped.');
            return;
        }

        $client = new \Google\Client();
        $client->setAuthConfig($credentialsPath);
        $client->addScope('https://www.googleapis.com/auth/firebase.remoteconfig');

        $httpClient = $client->authorize();
        $projectId = 'pawandpawspetstore';

        // Fetch current template
        $response = $httpClient->get("https://firebaseremoteconfig.googleapis.com/v1/projects/{$projectId}/remoteConfig");
        ...
        $template['parameters']['domain_url'] = [
            'defaultValue' => [
                'value' => $domainUrl
            ]
        ];
        ...
        $httpClient->put("https://firebaseremoteconfig.googleapis.com/v1/projects/{$projectId}/remoteConfig", [
            'headers' => [
        ...
        \Log::info("Firebase Remote Config updated with domain_url: {$domainUrl}");
    }
}
