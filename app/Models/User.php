...
class User extends Authenticatable implements HasMedia, MustVerifyEmail
{
    ...
    {
        return $this->morphOne(Address::class, 'addressable_id');
    }
    ...
    public function scopeEmployee($query, $userType = null)
    {
        if ($userType) {
            $query->role($userType);
        } else {
            $query->role('boarder'); // Default role if no user_type is provided
        }
    }

    public function scopeBranch($query)
    {
        $branch_id = request()->selected_session_branch_id ?? request()->branch_id;
        if (isset($branch_id)) {
            if (!collect($query->getQuery()->joins)->pluck('table')->contains('branch_employee')) {
                $query->join('branch_employee', 'users.id', '=', 'branch_employee.employee_id');
            }
            $query->where('branch_employee.branch_id', $branch_id);
        }
    }
