...
class CommonNotification extends Notification implements ShouldQueue
{
    ...
    public function __construct($type, $data)
    {
        ...
        foreach ($this->data as $key => $value) {
            $replaceValue = is_array($value) ? json_encode($value) : (string) $value;
            $templateDetail = str_replace('[[ ' . $key . ' ]]', $replaceValue, $templateDetail);
        }
        ...
        $this->subject = $templateData->subject ?? 'None';
    }
    ...
    public function Types($type)
    {
        switch ($type) {
            case 'new_booking':
                ...
                break;
            case 'accept_booking':
                return $this->type = 'Booking Request Accepted';
                break;
            case 'reject_booking':
                return $this->type = 'Booking Rejected';
                break;
            ...
            case 'order_placed':
                ...
                break;
            case 'order_accepted':
                return $this->type = 'Order Accepted';
                break;
