...
{
    $system['name'] = 'playpaws';
    $system['version'] = '1.0.0';
    $system['build_version'] = '1.2.2';
    return $system;
}
...
{
    try {
        $logo = \Illuminate\Support\Facades\DB::table('settings')->where('name', 'logo')->value('val');
        if ($logo) return $logo;
    } catch (\Exception $e) {}
    return asset('assets/images/logoIcon/logo.png');
}

function siteLogoDark()
{
    try {
        $logo = \Illuminate\Support\Facades\DB::table('settings')->where('name', 'dark_logo')->value('val');
        if ($logo) return $logo;
    } catch (\Exception $e) {}
    return asset('assets/images/logoIcon/logo_dark.png');
}

function siteFavicon()
{
    try {
        $favicon = \Illuminate\Support\Facades\DB::table('settings')->where('name', 'favicon')->value('val');
        if ($favicon) return $favicon;
    } catch (\Exception $e) {}
    return asset('assets/images/logoIcon/favicon.png');
}
...
function gs()
{
    ...
    if (!$general) {
        ...
        if ($general) {
            try {
                $settings = \Illuminate\Support\Facades\DB::table('settings')->pluck('val', 'name');
                if (isset($settings['app_name'])) {
                    $general->site_name = $settings['app_name'];
                }
                
                $primaryCurrency = \Illuminate\Support\Facades\DB::table('currencies')->where('is_primary', 1)->first();
                if ($primaryCurrency) {
                    $general->cur_text = $primaryCurrency->currency_code;
                    $general->cur_sym = $primaryCurrency->currency_symbol;
                }
                
                if (isset($settings['root_colors'])) {
                    $colors = json_decode($settings['root_colors'], true);
                    if(isset($colors['--bs-primary'])) {
                        $general->base_color = ltrim($colors['--bs-primary'], '#');
                    }
                    if(isset($colors['--bs-secondary'])) {
                        $general->secondary_color = ltrim($colors['--bs-secondary'], '#');
                    }
                }
            } catch (\Exception $e) {}
        }
...
function calculateAverageRating($averageRating)
{
    ...
    $ratingHtml = '<ul class="rating-list justify-content-center">';

    //full stars
    for ($i = 0; $i < $fullStars; $i++) {
        $ratingHtml .= '<li class="rating-list__item"><i class="fas fa-star"></i></li>';
    }

    //half star
    if ($halfStar > 0) {
        $ratingHtml .= '<li class="rating-list__item"><i class="fas fa-star-half"></i></li>';
    }

    //empty stars
    for ($i = 0; $i < $emptyStars; $i++) {
        $ratingHtml .= '<li class="rating-list__item"><i class="far fa-star"></i></li>';
    }
