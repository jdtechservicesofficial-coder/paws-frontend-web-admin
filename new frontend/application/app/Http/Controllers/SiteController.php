...
class SiteController extends Controller
{
    ...
    {
        $policy = \Illuminate\Support\Facades\DB::table('pages')->where('id', $id)->first();
        if(!$policy) abort(404);
    ...
    public function blogDetails($slug, $id){
        ...
        if(!$blog) abort(404);
        $pageTitle = $blog->name;
        $blogs = \Illuminate\Support\Facades\DB::table('blogs')->where('status', 1)->get();
        return view($this->activeTemplate.'blog_details',compact('blog','pageTitle', 'blogs'));
    }

    public function blogs(){
        $services = \Illuminate\Support\Facades\DB::table('system_services')->where('status', 1)->limit(5)->get();
        $blogs = \Illuminate\Support\Facades\DB::table('blogs')->where('status', 1)->get();
        $pageTitle = 'Blogs';
    ...
    public function serviceDetails($slug, $id){
        $service = \Illuminate\Support\Facades\DB::table('system_services')->where('id', $id)->first();
    ...
    public function removeCartItem(Request $request)
    {
     ...
     public function updateQuantity(Request $request)
     {
      ...
      public function getChecktout(){
        ...
        // Fetch Pawlly Logistic Zones and Tax
        $shippings = \Illuminate\Support\Facades\DB::table('logistic_zones')->get();
        $tax = \Illuminate\Support\Facades\DB::table('taxes')->where('module_type', 'products')->where('status', 1)->whereNull('deleted_at')->first();
        
        return view($this->activeTemplate.'cart.checkout',compact('gatewayCurrency','mobileCode','countries','pageTitle','shippings', 'tax'));
    ...
    public function productDetails($slug,$id){
        ...
        $productImages = \Illuminate\Support\Facades\DB::table('media')->where('model_type', 'Modules\Product\Models\Product')->where('model_id', $product->id)->get()->toArray();
       ...
       public function productFilter(Request $request)
    {
        ...
        $products = $query->whereHas('categories', function ($query) {
