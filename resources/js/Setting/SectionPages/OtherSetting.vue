<template>
  <form @submit="formSubmit">
    ...
    <div class="form-group mt-3">
      <label for="app_domain_url">App Domain URL <small>(Automatically syncs with Firebase Remote Config)</small></label>
      <input type="text" class="form-control" v-model="app_domain_url" id="app_domain_url" name="app_domain_url" :errorMessage="errors.app_domain_url" :errorMessages="errorMessages.app_domain_url" />
      <p class="text-danger" v-for="msg in errorMessages.app_domain_url" :key="msg">{{ msg }}</p>
    </div>
...
const setFormData = (data) => {
  resetForm({
    values: {
      ...
      app_domain_url: data.app_domain_url || ''
    }
  })
}
//validation
const validationSchema = yup.object({
  ...
  app_domain_url: yup.string().url('Must be a valid URL')
})
...
const { value: app_domain_url } = useField('app_domain_url')
