...
<head>
    ...
    @endforeach
    @if (setting('is_one_signal_notification') == 1 && !empty(setting('onesignal_app_id')))
        ...
        <div class="position-relative  {{ !isset($isBanner) ? 'iq-banner' : '' }} default ">
            ...
            <nav
                ...
                <div class="container-fluid navbar-inner">
                    ...
                    @php
                        $get_pages = \Modules\Page\Models\Page::where('status', 1)->get();
                    @endphp
                    <!-- <div class="menu">
...
@endforeach
                        ...
                        <div class="collapse navbar-collapse header-right-panel" id="navbarSupportedContent">
                            <ul class="iq-nav-menu list-unstyled">
                                <!-- @include('vendor.laravel-menu.custom-menu-items', [
                                    'items' => $horizontal_menu->roots(),
                                ]) -->
                                @foreach ($get_pages as $page)
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('pages', $page->slug) }}">{{ $page->name }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
            ...
            @if (!isset($isBanner))
                ...
                <div class="iq-navbar-header navs-bg-color" style="height: 7.5rem;">
